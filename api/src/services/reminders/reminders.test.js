...
describe('reminders', () => {
  ...
  scenario('creates a reminder', async () => {
    const result = await createReminder({
      input: {
        role: 'String',
        date: '2022-07-03T17:41:28Z',
        createdBy: 'String',
      },
    })

    expect(result.role).toEqual('String')
    expect(result.date).toEqual('2022-07-03T17:41:28Z')
    expect(result.createdBy).toEqual('String')
  })

  scenario('updates a reminder', async (scenario) => {
    const original = await reminder({ id: scenario.reminder.one.id })
  ...
  scenario('deletes a reminder', async (scenario) => {
    const original = await deleteReminder({ id: scenario.reminder.one.id })
